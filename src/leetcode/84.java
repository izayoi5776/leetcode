...
class Solution {
    public int largestRectangleArea(int[] heights) {
        ...
        if(heights!=null && heights.length>0){
            ...
            // 求最小值数组
            //int[][] M = new int[n][0];  // M[i][j] 保存从 i 到 j 子序列中的最小值。注意 j 含
            int[] M = new int[n];  // M[i][j] 保存从 i 到 j 子序列中的最小值。注意 j 含
            for(int i=n-1; i>=0; i--){
                M[i]= h[i];
                ret = Math.max(ret, M[i]);
                for(int j=i-1; j>=0; j--){
                    M[j] = h[j]<M[j+1] ? h[j] : M[j+1];
                    int S = (i-j+1) * M[j];
                    if(S>ret){
                        ret = S;
                    }
                    //System.out.printf("largestRectangleArea(%s)=%d M[%d][%d]=%d M=%s\n", Arrays.toString(h), ret, i, j, M[i][j], Arrays.deepToString(M), ret);
                }
            }
...
// -----------------------------------------------------------
	...
	static void t1(){
		tbase(10, new int[]{2,1,5,6,2,3});
		
	}
	static void t2(){
	    tbase(1, new int[]{1}); // 77 of 96
	}
	// 94 of 96 memory over
	static void t3(){
	    tbase(1, new int[]{
